...
def main(argv=None):
    ...
    if api_url == "":
        print("JUPYTERHUB_API_URL environment variable is not set. Exiting.",stream=sys.stderr)
        sys.exit(1)

    hub_auth = HubAuth(api_url=api_url)
    url = url_path_join(api_url, "slurmrestapi")
    ...
    sys.argv = sys.argv[1:]
    print(f"Running command: {cmd_path} with args: {sys.argv}")
